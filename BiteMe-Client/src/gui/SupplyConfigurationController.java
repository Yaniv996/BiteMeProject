...
public class SupplyConfigurationController {
    ...
    private void initialize() {
        ...
        SpinnerValueFactory<Integer> hourValueFactory = new SpinnerValueFactory.IntegerSpinnerValueFactory(8, 22);
    ...
    private void handleNextButtonAction() 
    {
        ...
        {
        	timeNotSpecError.setText("You must Specify a supply time!");
        ...
        else 
        {
            ...
            if (dateTimeString != null) 
            {
                ...
                if (flag==1) {
                	BiteOptions updateDis = new BiteOptions(UserDitales.toString(), BiteOptions.Option.UPDATE_DISCOUNT_COUNT);
                    // Send the request to the server to update the discount count in the database
                     ClientUI.chat.accept(updateDis);
    ...
    private void handleNextButtonAction() 
    {
        ...
        } else if (selfPickupNameField.getText().trim().isEmpty()) 
        {
        	timeNotSpecError.setVisible(false);
        	timeNotSpecError.setText("Full Name must be provided!");
            timeNotSpecError.setVisible(true);
            
        }else if (selfPickupPhoneField.getText().trim().isEmpty()) 
        {
        	timeNotSpecError.setVisible(false);
        	timeNotSpecError.setText("Phone number must be provided!");
            timeNotSpecError.setVisible(true);
            
        } else if (supplyMethodComboBox.getValue() == null) 
        {
        	timeNotSpecError.setVisible(false);
        	timeNotSpecError.setText("You must select a supply method!");
            timeNotSpecError.setVisible(true);
        }  
        else 
        {
    ...
    private void applyTimeBasedDiscount() {
        ...
        discounLabel.setText("10% Time Discount!");
